...
| B | **Активация в воркере** (dispatch event_type `chunk`/`reduce`, маршрутизация large-PR через map-reduce; предлагается за фиче-флагом OFF) | Мерж в `main` = автодеплой. Включать map-reduce в живой воркер — «go-live»-переключение: осознанный шаг после проверки A на стейджинге |
| C | **Калибровка порога/размера чанка на GLM-5/GLM-4.7** (Q2 числа) | Требует замера латентности vs размер на реальной модели |
| Q8 | **Метод оценки качества** ревью чанка (для калибровки промпта A) | Нужно решение PO |
