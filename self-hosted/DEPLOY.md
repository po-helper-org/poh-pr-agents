...
`RELIABILITY_REPOS` — репозитории (через запятую), которые свипер сверяет и
дозапускает; без него sweeper бездействует. Остальные настройки (LLM Gateway,
интервалы) имеют дефолты — трогать не нужно (полный список — в конце `GO-LIVE.md`).
