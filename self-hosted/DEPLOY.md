...
RELIABILITY_REPOS=po-helper-org/poh-pr-agents
```

`RELIABILITY_REPOS` — область для **свипера** (бэкстопа на пропущенные webhook'и):
- список `owner/repo,owner/repo2` — сверять ровно эти;
- **пусто → org-wide**: свипер сам обходит **все репозитории всех установок App**
  (несколько орг/аккаунтов сразу, включая новые репо) — руками список не ведёшь.
...
2. **`RELIABILITY_REPOS` оставь пустым** → свипер авто-обнаружит все репо всех
   установок. Ничего перечислять не нужно, новые репо подхватываются сами.
