...
| В PR повторяется коммент о провале; `dead_letter_total` растёт | аутейдж/неверный ключ Z.AI, таймаут | Класс сбоя прямо в комменте (`GatewayUnavailable` / `TimeoutError` / `ValueError`); `metrics`: `gateway_provider_failure`, `gateway_unavailable` | Починить ключ/endpoint/лимит Z.AI. После починки sweeper дозапустит; или вручную `/review` в комменте |
