...
> 🗺️ Как устроен конвейер и как чинить сбои — [`ARCHITECTURE.md`](ARCHITECTURE.md)
> (диаграммы + плейбук диагностики). Полная установка с нуля (App, секреты, Dokploy,
> нагрузка) — [`DEPLOY.md`](DEPLOY.md); этот файл фокусируется на переключении и откате.
...
Охват свипера (какие открытые PR сверять):
```
RELIABILITY_REPOS="owner/repo,owner/repo2"  # точные репо; ИЛИ маска "owner/*" (все репо орг, вкл. новые), можно "po-helper-org/*,ai-oxudevelopment/*" / "*"; ПУСТО → org-wide (все установки App)
```
