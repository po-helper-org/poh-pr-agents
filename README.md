...
## После первого ревью: как проверить, что правки исправлены

Типовой вопрос: *«Ревью прошло, я внёс правки — что теперь сделать, чтобы убедиться, что замечания закрыты?»* Ниже цикл «правка → повторная проверка».

### Важно понимать заранее

- **Ревью обновляется НА МЕСТЕ, а не новым комментарием.** У `/review` включён `persistent_comment` (дефолт): результат всегда лежит в **одном верхнем комментарии** `PR Reviewer Guide 🔍`, и каждый прогон переписывает его. Вниз бот кидает только короткую заметку-указатель **«Persistent review updated to latest commit `<sha>`»** — это не «пустой ответ», а ссылка на обновлённое ревью. Нового комментария не ждите.
- **Замечания живут в ДВУХ комментариях**, и каждый инструмент обновляет только свой:

  | Хотите перепроверить… | Запустите | Комментарий |
  |---|---|---|
  | focus areas, соответствие тикетам, security | `/review` | `PR Reviewer Guide 🔍` |
  | конкретные code-предложения | `/improve` | `PR Code Suggestions ✨` |

  Если правка была по предложению из `/improve` — надо отдельно написать **`/improve`**, иначе тот блок останется со старым содержимым. `/review` его не трогает.
- **`/review` — это не чек-лист «исправлено/нет».** Он не сравнивает с прошлым прогоном, а заново ревьюит текущий diff. Проверка = глазами сверить: замечание **пропало** из обновлённого ревью → исправлено ✅; **осталось** → не поправили или ложное срабатывание ❌.

### Цикл

1. **Запушьте правки в ветку PR** (`git push`).
2. **Перезапустите ревью:**
   - **Режим A (GitHub Actions):** триггера на `push`/`synchronize` нет — после пуша **вручную** напишите в PR `/review` (и `/improve`, если правили предложения).
   - **Режим B (self-hosted):** при `handle_push_trigger = true` ревью **перезапустится само** на новый коммит (см. `push_commands` в [`.pr_agent.toml`](self-hosted/.pr_agent.toml)); вручную дублировать не обязательно.
3. **Дождитесь заметки** «Persistent review updated to latest commit `<sha>`» и сверьте: `<sha>` в конце должен совпадать с вашим свежим коммитом. Не совпал — ревью ещё не пересобралось, подождите (self-hosted обрабатывает webhook ~30–60 сек).
4. **Откройте сам persistent-комментарий** (ссылка «Persistent review» в заметке), а не заметку-указатель — там актуальное ревью по вашему коду.
5. **Сверьте пункты.** Ушло замечание — закрыто; осталось — чините дальше.

### Слепое пятно

`/review` видит **только diff PR**, а не весь репозиторий. Поэтому замечания вида «команда/файл может не существовать» будут повторяться, даже если файл в репо есть — просто он вне диффа. Это ложное срабатывание: игнорируйте или закройте ответным комментарием.

### Шпаргалка

```
правка → git push
      → /review (+ /improve, если правили предложения)   # режим A — вручную; режим B — авто
      → ждём "Persistent review updated to latest commit <ваш sha>"
      → открываем "Persistent review" (верхний комментарий)
      → сверяем: замечание ушло = исправлено; осталось = чиним или это ложное
```
