...
## Протокол взаимодействия агентов v1

PR-Agent — один из трёх сервисов контура производства и соблюдает общий
[протокол](https://github.com/po-helper-org/.github/blob/main/AGENT-PROTOCOL.md).
Отсюда два поведения, которых нет у Qodo «из коробки».

**Стоп-метки.** Событие на PR с меткой `pr-closer:closed` либо `agents:off`
отбрасывается на разборе payload — до обращения к API и тем более к модели.
Смысл ровно в этом: не тратить бюджет на PR, который уже закрыт для агентов.

- `pr-closer:closed` ставит PR-Closer, когда цикл правок завершён. Без проверки
  этой метки PR-Agent выдал бы четвёртый круг там, где агент уже сказал стоп-слово,
  и инвариант «после стоп-слова никто не пишет в ветку» держался бы
  на добросовестности, а не на механизме.
- `agents:off` ставит человек, когда забирает PR себе. Действует и на явную
  команду в комментарии: рубильник, который обходится командой `/review`,
  рубильником не является.

Снятие метки возвращает PR в оборот — перезапускать ничего не нужно.

**Севериность замечаний.** Каждое замечание начинается с машиночитаемой строки
`severity: blocker|major|nit`. Этим контрактом управляется цикл PR-Closer: круг
правок продлевают только `blocker` и `major`, а `nit` — нет.

При неуверенности класс понижается. Ложный `blocker` гоняет цикл правок впустую,
ложный `nit` просто уезжает в follow-up: цена этих ошибок разная.

Без разметки PR-Closer классифицирует сам — слабой моделью и с завышением класса,
и тогда лимит из трёх кругов расходуется на неймингах.
